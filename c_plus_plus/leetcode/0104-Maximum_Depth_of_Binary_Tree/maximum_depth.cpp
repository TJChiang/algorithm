...
class Solution {
public:
    int maxDepth(structures::TreeNode* root) {
    ...
    int maxDepthIteration(structures::TreeNode* root) {
        if (root == nullptr) return 0;
        int result = 0;
        queue<structures::TreeNode*> node_queue;
        ...
        while (!node_queue.empty()) {
            ...
            for (int i = 0; i < length; i++) {
                structures::TreeNode* cur = node_queue.front();
                node_queue.pop();
...
{
    structures::BinaryTree bt1(vector<int>{3,9,20,INT_NULL_,INT_NULL_,15,7});
    structures::BinaryTree bt2(vector<int>{1,INT_NULL_,2});
    Solution sol;
