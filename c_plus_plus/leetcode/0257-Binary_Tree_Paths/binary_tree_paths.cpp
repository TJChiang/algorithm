...
class Solution {
public:
    void traversal_expend(structures::TreeNode* cur, vector<int> path, vector<string>& result) {
    ...
    vector<string> binaryTreePaths_expend(structures::TreeNode* root) {
    ...
    void traversal(structures::TreeNode* cur, string path, vector<string>& result) {
    ...
    vector<string> binaryTreePaths(structures::TreeNode* root) {
        vector<string> result;
        string path;
        if (root == nullptr) return result;
...
int main()
{
    vector<structures::BinaryTree> questions = {
            structures::BinaryTree(vector<int> {1, 2, 3, INT_NULL_, 5}),
            structures::BinaryTree(vector<int> {1}),
    };
    Solution sol;
